            ...
            if start_time is None:
                for h in history:
                    if self.check_user(h['user']):
                        if h['user'] in self.editors:
                            self.editors[h['user']]['counter'] += 1
                            self.editors[h['user']]['edits'].append(
                                datetime.strptime(
                                    h['timestamp'][0:10], "%Y-%m-%d"))
                        else:
                            self.editors[h['user']] = {'counter': 1,
                             'invited':  False, 'edits': list()}
...
class DAO:
    ...
    def get_editors(self):
        ...
        for key in self.db:
            if (self.db[key]['counter'] >= threshold
             and not self.db[key]['invited']):
